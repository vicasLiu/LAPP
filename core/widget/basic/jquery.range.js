(function($, window, document, undefined) {
	...
	jRange.prototype = {
		...
		onDrag: function(pointer, e) {
			...
			if (pointer.changedTouches && pointer.changedTouches.length) {
				pointer = pointer.changedTouches[0];
			}
			var position = pointer.clientX - this.domNode.offset().left;
		...
		setPosition: function(pointer, position, isPx, animate) {
			...
			// pointer.css({'left':Math.round(position - circleWidth)})
		...
		showPointerValue: function(pointer, position, animate) {
			...
			$('#'+id).attr('data-value',text);
			$('.pointer-label').html(text)
